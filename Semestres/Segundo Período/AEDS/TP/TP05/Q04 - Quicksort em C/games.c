...
// VARIAVEIS GLOBAIS PARA CONTAR COMPARACOES E MOVIMENTACOES
int comparacoes = 0;
int movimentacoes = 0;
...
//FUNCAO PARA TROCAR DOIS GAMES
void swapGame(Game *a, Game *b) {
    if (a != b) { // SO CONTA COMO MOVIMENTACAO SE REALMENTE TROCAR
        movimentacoes += 3; // UMA TROCA ENVOLVE 3 MOVIMENTACOES (temp = *a, *a = *b, *b = temp)
        Game temp = *a;
        *a = *b;
        *b = temp;
    }
...
//FUNCAO PARA COMPARAR DUAS DATAS E IDS
int compararGames(Game a, Game b) {
    comparacoes++; // INCREMENTA CONTADOR DE COMPARACOES
    
    // COMPARA POR ANO
    if (a.releaseDate.ano < b.releaseDate.ano) return -1;
    if (a.releaseDate.ano > b.releaseDate.ano) return 1;
    
    // SE O ANO FOR IGUAL, COMPARA POR MES
    if (a.releaseDate.mes < b.releaseDate.mes) return -1;
    if (a.releaseDate.mes > b.releaseDate.mes) return 1;
    
    // SE O MES FOR IGUAL, COMPARA POR DIA
    if (a.releaseDate.dia < b.releaseDate.dia) return -1;
    if (a.releaseDate.dia > b.releaseDate.dia) return 1;
    
    // SE AS DATAS FOREM IGUAIS, COMPARA POR ID
...
void quicksortRec(Game *array, int esq, int dir) {
    ...
    while (i <= j) {
        while (compararGames(array[i], pivo) < 0) i++;
        while (compararGames(array[j], pivo) > 0) j--;
        if (i <= j) {
            swapGame(&array[i], &array[j]);
            i++;
            j--;
        }
    }
...
int main()
{
    ...
    Game listaGamesOrdenada[5000];
    int indexListaGamesOrdenada = 0;

    // LE ATÉ ENCONTRAR FIM E CRIA UM ARRAY COM OS ELEMENTOS DESEJADOS
    while (strcmp(texto, fim))
    {
        ...
        for(int i=0; i < 5000; i++) {
            ...
            if(listaGames[i].id == id) {
                listaGamesOrdenada[indexListaGamesOrdenada] = listaGames[i];
                indexListaGamesOrdenada++;
            }
        }
        fgets(texto, sizeof(texto), stdin);
    }

    //ORDENA O ARRAY E MEDE O TEMPO
    comparacoes = 0;
    movimentacoes = 0;
    
    clock_t inicio = clock();
    quicksort(listaGamesOrdenada, indexListaGamesOrdenada);
    clock_t fimTempo = clock();
    
    double tempoExecucao = ((double)(fimTempo - inicio)) / ((double)CLOCKS_PER_SEC);
    
    // CRIA O ARQUIVO DE LOG
    FILE *logFile = fopen("885810_quicksort.txt", "w");
    if (logFile != NULL) {
        fprintf(logFile, "885810\t%d\t%d\t%.6f\n", comparacoes, movimentacoes, tempoExecucao);
        fclose(logFile);
    }

    for(int i=0; i < indexListaGamesOrdenada; i++) {
        if(listaGamesOrdenada[i].price == 0.0) {
            printf("=> %d ## %s ## %02d/%02d/%d ## %d ## %.1f ## ", listaGamesOrdenada[i].id, listaGamesOrdenada[i].nome, listaGamesOrdenada[i].releaseDate.dia, listaGamesOrdenada[i].releaseDate.mes, listaGamesOrdenada[i].releaseDate.ano, listaGamesOrdenada[i].estimatedOwners, listaGamesOrdenada[i].price);
        } else {
            printf("=> %d ## %s ## %02d/%02d/%d ## %d ## %.2f ## ", listaGamesOrdenada[i].id, listaGamesOrdenada[i].nome, listaGamesOrdenada[i].releaseDate.dia, listaGamesOrdenada[i].releaseDate.mes, listaGamesOrdenada[i].releaseDate.ano, listaGamesOrdenada[i].estimatedOwners, listaGamesOrdenada[i].price);
        }
        listarArray(listaGamesOrdenada[i], 1);
        printf(" ## %d ## %.1f ## %d ## ", listaGamesOrdenada[i].metacriticScore, listaGamesOrdenada[i].userScore, listaGamesOrdenada[i].achievements);
        listarArray(listaGamesOrdenada[i], 2);
        printf(" ## ");
        listarArray(listaGamesOrdenada[i], 3);
        printf(" ## ");
        listarArray(listaGamesOrdenada[i], 4);
        printf(" ## ");
        listarArray(listaGamesOrdenada[i], 5);
        printf(" ## ");
        listarArray(listaGamesOrdenada[i], 6);
        printf(" ##\n");
    }
