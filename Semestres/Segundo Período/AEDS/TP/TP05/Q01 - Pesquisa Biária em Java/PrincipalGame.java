...
public class PrincipalGame {
    ...
    //FUNCAO DA PESQUISA BINARIA PARA FAZER A CONSUKLTA
    public static boolean pesqBin(Game[] vet, int qtdElementos, String x) {
        boolean resp = false;
        int dir = qtdElementos-1, esq = 0, meio;

        while (esq <= dir) {
            meio = (esq + dir) / 2;
            String meioVal = vet[meio].getNome();

            //CRIA UMA VARIAVEL DE COMPARACAO
            //ELA RETORNA UM INTEIRO NEGATIVO SE O A STRING X FOR MENOR QUE meioVal
            //ELA RETORNA UM INTEIRO POSITIVO SE O A STRING X FOR MAIOR QUE meioVal
            //ELA RETORNA ZERO SE O A STRING X FOR IGUAL A meioVal
            int comparacao = x.compareTo(meioVal);
            numComparacoes++;

            if (comparacao == 0) {
                //SE FOR 0 É PQ É IGUAL
                resp = true;
                esq = dir + 1;
    ...
    //FUNCAO RESPONSAVEL POR ORDENAR O ARRAY PELO NOME
    public static Game[] sort(Game array[], int n) {
		for (int i = 0; i < (n - 1); i++) {
			int menor = i;
			for (int j = (i + 1); j < n; j++){
    ...
    public static void main(String args[]) throws IOException {
        ...
        try (Scanner sc = new Scanner(System.in)) {
            ...
            //EXECUTA A ORDENACAO POR SELECAO NECESSARIA PARA O BINARY SEARCH
            listaGames = sort(listaGames, index);
            ...
            while (contadorFim < 2 && contadorFim >= 1) {
                if (!equals(id, "FIM")) {
                    if(pesqBin(listaGames, index, id))
            ...
            try (FileWriter writer = new FileWriter("885810_binaria.txt")) {
                writer.write("885810\t" + tempoExecucao + "\t" + numComparacoes);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
