...
public class PrincipalGame {
    ...
    public static int comparar(Game a, Game b) {
        comparacoes++;
        // PRIMEIRO CRITERIO: estimatedOwners
    ...
    public static void main(String args[]) throws IOException {
        ...
        //ORDENA O LISTA GAMES FINAL COM MEDIÇÃO DE TEMPO
        long inicioTempo = System.currentTimeMillis();
        heapSort(listaGamesFinal, indexListaGamesFinal);
        long fimTempo = System.currentTimeMillis();
        long tempoExecucao = fimTempo - inicioTempo;

        try (FileWriter writer = new FileWriter("885810_heapsort.txt")) {
            writer.write("885810\t" + comparacoes + "\t" + movimentacoes + "\t" + tempoExecucao + "\n");
        } catch (IOException e) {
            e.printStackTrace();
        }

        //IMPRIME DE FORMA ORDENADA
        for (int i = 0; i < indexListaGamesFinal; i++) {
