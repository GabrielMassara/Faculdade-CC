...
public class PrincipalGame {
    ...
    public static void main(String args[]) throws IOException {
        ...
        try (Scanner sc = new Scanner(System.in)) {
            ...
            // CRIA MINHA HASH TABLE E PREENCHE ELA NA ORDEM CORRETA DOS IDS
            ...
            // LE ATE ENCONTRAR "FIM" e pesquisa na hash
            while (contadorFim < 2 && contadorFim >= 1) {
                if (!equals(id, "FIM")) {
                    numComparacoes = 0;
                    boolean encontrado = hash.pesquisar(id);
                    totalComparacoes += numComparacoes;
                    
                    int posicao = hash.getUltimaPosicaoPesquisada();
                    System.out.println(id + ": " + " (Posicao: " + posicao + ") " + (encontrado ? "SIM" : "NAO"));
            ...
            try (FileWriter writer = new FileWriter("885810_hashReserva.txt")) {
...
class HashTable {
   Game tabela[];
   int m1, m2, m, reserva;
   final Game NULO = null;
   private int ultimaPosicaoPesquisada;
   ...
   public boolean pesquisar(String nome) {
      boolean resp = false;
      int pos = h(nome);
      ultimaPosicaoPesquisada = pos;
      
      PrincipalGame.numComparacoes++;
      if (tabela[pos] != NULO && tabela[pos].getNome().equals(nome)) {
         resp = true;
      } else if (tabela[pos] != NULO) {
         // PROCURA NA AREA DE RESERVA
         for (int i = 0; i < reserva; i++) {
            PrincipalGame.numComparacoes++;
            if (tabela[m1 + i] != NULO && tabela[m1 + i].getNome().equals(nome)) {
               ultimaPosicaoPesquisada = m1 + i;
               resp = true;
               i = reserva;
            }
         }
   ...
   public int getUltimaPosicaoPesquisada() {
      return ultimaPosicaoPesquisada;
   }
}
