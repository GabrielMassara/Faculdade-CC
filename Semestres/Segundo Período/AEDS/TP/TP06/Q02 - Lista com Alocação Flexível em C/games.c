...
//ESTRUTURA PARA A CELULA
typedef struct Celula {
    Game game;
    struct Celula* prox;
} Celula;

Celula* novaCelula(Game game) {
...
//VOU INICIALIZAR A MINHA LISTA COM UM NÓ CABECA
void inicializar() {
    Game jogoCabeca;
    primeiro = novaCelula(jogoCabeca);
    ultimo = primeiro;
}

void inserirInicio(Game game) {
    Celula* nova = novaCelula(game);
...
void inserir(Game game, int posicao) {
    Celula* nova = novaCelula(game);
    
    Celula* i = primeiro;
    int index = 0;
    //PEGO O ELEMENTO ANTERIOR A SER INSERIDO
    for(; index < posicao; i=i->prox, index++);

    Celula* aux = i->prox;
    i->prox = nova;
    nova->prox = aux;

    //SE ADICIONAR NO FINAL ANDA COM O PONTEIRO ULTIMO
    if(nova->prox == NULL) {
        ultimo = nova;
    }
...
void remover(int posicao) {
    Celula* i = primeiro;
    int index = 0;
    //PEGA O ELEMENTO DA POSICAO ANTERIOR
    for(; index < posicao; i=i->prox, index++);
    Celula* aux = i->prox;
    i->prox = i->prox->prox;
    aux->prox = NULL;

    //SE REMOVER O ULTIMO, VOLTA COM O PONMTEIRO ULTIMO PARA TRAS
    if(ultimo == aux) {
        ultimo = i;
    }
...
void imprimir() {
    int index = 0;
    for(Celula* i = primeiro->prox; i!=NULL; i=i->prox, index++) {
        if(i->game.price == 0.0) {
            printf("[%d] => %d ## %s ## %02d/%02d/%d ## %d ## %.1f ## ", index, i->game.id, i->game.nome, i->game.releaseDate.dia, i->game.releaseDate.mes, i->game.releaseDate.ano, i->game.estimatedOwners, i->game.price);
        } else {
            printf("[%d] => %d ## %s ## %02d/%02d/%d ## %d ## %.2f ## ", index, i->game.id, i->game.nome, i->game.releaseDate.dia, i->game.releaseDate.mes, i->game.releaseDate.ano, i->game.estimatedOwners, i->game.price);
        }
        listarArray(i->game, 1);
        printf(" ## %d ## %.1f ## %d ## ", i->game.metacriticScore, i->game.userScore, i->game.achievements);
        listarArray(i->game, 2);
        printf(" ## ");
        listarArray(i->game, 3);
        printf(" ## ");
        listarArray(i->game, 4);
        printf(" ## ");
        listarArray(i->game, 5);
        printf(" ## ");
        listarArray(i->game, 6);
        printf(" ##\n");
    }
}
...
char* pegarParametro(char texto[], int inicio, int parametro) {
    //SE PARAMETRO FOI IGUAL A 1, ELE PEGA O PRIMEIRO NUMERO. SE FOR 2 ELE PEGA O SEGUNDO
    int tamanho = length(texto) - inicio;
    char *resultado = (char*)malloc((tamanho + 1) * sizeof(char));
    int indexResultado = 0;
    int i = inicio;
    //ESSE FOR EXECUTA OBRIGATORIAMENTE, POIS SE FOR O PARAMETRO 1 ELE JA ADICIONA
    //MAS SE FOR O PARAMETRO 2 ELE JA PEGA O INDEX ANTERIOR AO NECESSARIO
    for(; texto[i] != ' ' && i < length(texto); i++) {
        if(parametro == 1) {
            resultado[indexResultado] = texto[i];
            indexResultado++;
        }
    }
    //AVANCA 1 PARA SAIR DO ESPACO E IR PARA O PRIMEIRO CARACTERE
    i++;
...
Game buscaGame(int id, Game listaGames[]) {
...
int main()
{
    ...
    fgets(texto, sizeof(texto), stdin);
    int n = stringParaInteiro(texto);

    for (int i = 0; i < n; i++) {
        fgets(texto, sizeof(texto), stdin);

        //PEGA A OPERACAO SEMPRE COM OS DOIS PRIMEIROS CARACTERES
        char operacao[3] = {texto[0], texto[1], '\0'};
        char II[3] = "II";
        char IF[3] = "IF";
        char RI[3] = "RI";
        char RF[3] = "RF";
        ...
        if(strcmp(operacao, II) == 0) {
            char* idStr = pegarParametro(texto, 3, 1);
            int id = stringParaInteiro(idStr);
            free(idStr);
            inserirInicio(buscaGame(id, listaGames));
        } else if(strcmp(operacao, IF) == 0) {
            char* idStr = pegarParametro(texto, 3, 1);
            int id = stringParaInteiro(idStr);
            free(idStr);
            inserirFim(buscaGame(id, listaGames));
        } else if(strcmp(operacao, RI) == 0) {
            removerInicio();
        } else if(strcmp(operacao, RF) == 0) {
            removerFim();
        } else if(strcmp(operacao, RA) == 0) {
            char* posStr = pegarParametro(texto, 3, 1);
            int pos = stringParaInteiro(posStr);
            free(posStr);
            remover(pos);
        } else if(strcmp(operacao, IA) == 0) {
            //PEGA A POSICAO E O ID DO JOGO
            char* posStr = pegarParametro(texto, 3, 1);
            int pos = stringParaInteiro(posStr);
            free(posStr);
            char* idStr = pegarParametro(texto, 3, 2);
            int id = stringParaInteiro(idStr);
            free(idStr);
            inserir(buscaGame(id, listaGames), pos);
        }
    }
