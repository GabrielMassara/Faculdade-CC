...
void imprimir() {
    int index = 0;
    for(Celula* i = topo->prox; i!=NULL; i=i->prox, index++) {
        if(i->game.price == 0.0) {
...
int main()
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        char operacao[2] = {texto[0], '\0'};
        char I[2] = "I";
        char R[2] = "R";

        if(strcmp(operacao, I) == 0) {
            char* idStr = pegarParametro(texto, 2, 1);
            int id = stringParaInteiro(idStr);
            free(idStr);
            inserir(buscaGame(id, listaGames));
