...
public class PrincipalGame {
    ...
    public static void main(String args[]) throws IOException {
        ...
        try (Scanner sc = new Scanner(System.in)) {
            ...
            // CRIA MINHA ÁRVORE DE ÁRVORE E PREENCHE ELA NA ORDEM CORRETA DOS IDS
            ...
            try (FileWriter writer = new FileWriter("885810_arvoreArvore.txt")) {
...
// NO PARA A PRIMEIRA ARVORE
class NoPrimario {
    public NoPrimario esq;
    public NoPrimario dir;
    public int chave;
    public NoSecundario raizSecundaria; // RAIZ DA ARVORE ASSOCIADA
...
class NoSecundario {
    public NoSecundario esq;
    public NoSecundario dir;
    public Game elemento;

    NoSecundario(Game elemento) {
        this.esq = null;
        this.dir = null;
...
class ArvoreDeArvore {
    public NoPrimario raizPrimaria;
    public String caminhoPesquisa;

    ArvoreDeArvore() {
        this.raizPrimaria = null;
        this.caminhoPesquisa = "";
        // FUNCAO PARA CONSTRUIR A PRIMEIRA ARVORE
    ...
    private void construirPrimeiraArvore() {
        ...
        for (int chave : chaves) {
            raizPrimaria = inserirNoPrimario(chave, raizPrimaria);
        }
    }

    // INSERE UM NO NA PRIMEIRA ARVORE
    private NoPrimario inserirNoPrimario(int chave, NoPrimario no) {
        if (no == null) {
            no = new NoPrimario(chave);
        } else if (chave < no.chave) {
            no.esq = inserirNoPrimario(chave, no.esq);
        } else if (chave > no.chave) {
            no.dir = inserirNoPrimario(chave, no.dir);
        }
        return no;
    }

    // BUSCA UM NO COM A CHAVE ESPECIFICADA
    private NoPrimario buscarNoPrimario(int chave, NoPrimario no) {
        if (no == null) {
            return null;
        } else if (chave == no.chave) {
            return no;
        } else if (chave < no.chave) {
            return buscarNoPrimario(chave, no.esq);
        } else {
            return buscarNoPrimario(chave, no.dir);
        }
    }

    // FUNCAO PARA INSERIR UM GAME NA ESTRUTURA
    public void inserir(Game game) {
        int chave = game.getEstimatedOwners() % 15;
        NoPrimario noPrimario = buscarNoPrimario(chave, raizPrimaria);
        
        if (noPrimario != null) {
            noPrimario.raizSecundaria = inserirNoSecundario(game, noPrimario.raizSecundaria);
        }
    }

    // INSERE O GAME NA ARVORE SECUNDARIA
    private NoSecundario inserirNoSecundario(Game game, NoSecundario no) {
        if (no == null) {
            no = new NoSecundario(game);
        } else if (game.getNome().compareTo(no.elemento.getNome()) < 0) {
            no.esq = inserirNoSecundario(game, no.esq);
        } else if (game.getNome().compareTo(no.elemento.getNome()) > 0) {
            no.dir = inserirNoSecundario(game, no.dir);
        }
        return no;
    }

    // FUNCAO PARA PESQUISAR UM JOGO PELO NOME
    public boolean pesquisar(String nome) {
        caminhoPesquisa = "raiz ";
        return mostrarPrimaria(nome, raizPrimaria);
    }

    // PERCORRE A PRIMEIRA ARVORE (MOSTRAR PRE-ORDEM)  
    private boolean mostrarPrimaria(String nome, NoPrimario no) {
        if (no == null) {
            return false;
        }
        
        // VERIFICA A ARVORE SECUNDARIA DO NO ATUAL PRIMEIRO
        if (no.raizSecundaria != null) {
            boolean encontrado = mostrarSecundaria(nome, no.raizSecundaria);
            if (encontrado) return true;
        }
        
        // PERCORRE ESQUERDA
        caminhoPesquisa += " ESQ";
        boolean encontrado = mostrarPrimaria(nome, no.esq);
        if (encontrado) return true;
        
        // PERCORRE DIREITA  
        caminhoPesquisa += " DIR";
        return mostrarPrimaria(nome, no.dir);
    }

    // FUNCAO PARA PERCORRER A SEGUNDA ARVORE
    private boolean mostrarSecundaria(String nome, NoSecundario no) {
        if (no == null) {
            return false;
        }
        
        PrincipalGame.numComparacoes++;
        if (nome.equals(no.elemento.getNome())) {
            return true;
        } else if (nome.compareTo(no.elemento.getNome()) < 0) {
            caminhoPesquisa += " esq";
            return mostrarSecundaria(nome, no.esq);
        } else {
            caminhoPesquisa += " dir";
            return mostrarSecundaria(nome, no.dir);
        }
    }
